...
const mostBlogs = (blogs) => {
  ...
  const mostBloggedAuthor = Object.keys(authorBlogsCount).reduce(
    (previous, current) =>
  ...
  return {
    ...
    blogs: authorBlogsCount[mostBloggedAuthor],
  };
};
...
const mostLikes = (blogs) => {
  ...
  for (const blog of blogs) {
    authorTotalLikesCount[blog.author] =
      typeof authorTotalLikesCount[blog.author] !== "undefined"
        ? authorTotalLikesCount[blog.author] + blog.likes
        : blog.likes;
  }
  const mostLikedAuthor = Object.keys(authorTotalLikesCount).reduce(
    (previous, current) =>
